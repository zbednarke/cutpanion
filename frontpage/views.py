...
def polls_list(request):
    ...
    return render(request, 'frontpage/polls_list.html', context)
...
def polls_add(request):
    if request.user.has_perm('frontpage.add_poll'):
        if request.method == 'POST':
            ...
            if form.is_valid:
                ...
                return redirect('frontpage:list')
        else:
            form = PollAddForm()
        context = {
            'form': form,
        }
        return render(request, 'frontpage/add_poll.html', context)
    else:
...
def polls_edit(request, poll_id):
    ...
    return render(request, "frontpage/poll_edit.html", {'form': form, 'poll': poll})
...
def add_choice(request, poll_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid:
            ...
            return redirect('frontpage:edit', poll.id)
    else:
        form = ChoiceAddForm()
    context = {
        'form': form,
    }
    return render(request, 'frontpage/add_choice.html', context)
...
def poll_detail(request, poll_id):
    ...
    if not poll.active:
        return render(request, 'frontpage/poll_result.html', {'poll': poll})
    loop_count = poll.choice_set.count()
    context = {
        'poll': poll,
        'loop_time': range(0, loop_count),
    }
    return render(request, 'frontpage/poll_detail.html', context)
...
def poll_vote(request, poll_id):
    ...
    if choice_id:
        ...
        return redirect("frontpage:detail", poll_id)
    return render(request, 'frontpage/poll_result.html', {'poll': poll})
